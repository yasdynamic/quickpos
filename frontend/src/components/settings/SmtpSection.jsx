...
export default function SmtpSection() {
  ...
  const runTest = async () => {
    ...
    setTestResult(null);
    try {
      // make sure latest config saved before test
      await save({ smtp: form });
      const res = await api.post("/settings/smtp/test", { to: testTo });
      setTestResult(res.data);
      if (res.data.status === "sent") {
        toast.success(`Email de test envoyé à ${res.data.to}`);
      } else {
  ...
  return (
    <section className="rounded-md border border-[#E5E7EB] bg-white p-6 max-w-3xl">
      ...
      <p className="mb-4 text-sm text-[#4B5563]">
        Configurez votre propre serveur SMTP pour l&apos;envoi des rapports. Si activé,
        il sera utilisé en priorité sur Resend.
      </p>
      ...
      <div className="mt-6 flex flex-wrap gap-2">
        <button
          ...
          onClick={submit}
          disabled={saving}
          className="flex-1 h-14 rounded-md bg-[#002FA7] text-base font-bold uppercase tracking-wider text-white hover:bg-[#002277] active:scale-[0.98] disabled:opacity-50"
        >
